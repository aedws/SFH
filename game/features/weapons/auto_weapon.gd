...
func _process(delta: float) -> void:
	...
	if burst_remaining > 0:
		...
		burst_remaining -= 1
		cooldown = (
			float(current_balance.get(&"burst_interval_sec", 0.1))
			if burst_remaining > 0
			else float(current_balance.get(&"fire_interval_sec", 0.72))
		)
		return
...
func _find_nearest_enemy() -> Node2D:
	var nearest: Node2D
	var target_range := float(current_balance.get(&"target_range_px", 760.0))
...
func _spawn_projectile(direction: Vector2) -> void:
	...
	if projectile == null:
		return
	var damage := float(current_balance.get(&"damage", 1.0))
