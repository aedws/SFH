...
@export_file("*.csv") var locked_csv_path := "res://game/features/loot_tables/data/loot_table.csv"
@export var locked_csv_payload: Resource
@export_multiline var live_csv_url := "https://docs.google.com/spreadsheets/d/1dtQKVZiMf7VRFWrVnaL3BqzR0g4ZgEG6ueH9RIN3xqM/gviz/tq?tqx=out:csv&headers=1&sheet=LootTable"
@export_range(1.0, 60.0, 0.5) var live_refresh_seconds := 3.0
@export var fallback_to_locked_csv := true
@export_range(1, 10, 1) var briefing_item_count := 3


func validation_errors() -> PackedStringArray:
	var errors := PackedStringArray()
	if locked_csv_path.is_empty():
		errors.append("확정 LootTable CSV 경로가 비어 있습니다.")
	if (
		...
		errors.append("확정 LootTable CSV의 Web 내장 데이터가 유효하지 않습니다.")
	if source_mode == SourceMode.LIVE_GOOGLE_SHEET and live_csv_url.is_empty():
		errors.append("실시간 모드에는 LootTable Sheet 공개 CSV URL이 필요합니다.")
