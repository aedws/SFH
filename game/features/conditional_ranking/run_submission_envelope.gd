...
func validate(envelope: Dictionary) -> Dictionary:
	...
	if not payload.get(&"season_context", {}) is Dictionary:
		return _invalid(&"invalid_payload", "시즌 컨텍스트 형식 오류")
	for key in [&"elapsed_seconds", &"kills", &"boss_kills", &"recovered_value", &"reward_multiplier", &"penalty_score"]:
		var value: Variant = payload.get(key, 0)
		if not (value is int or value is float) or not is_finite(float(value)):
			return _invalid(&"invalid_metrics", "런 수치 형식 오류")
	if schema == 1 and (int(payload.get(&"boss_kills", 0)) != 0 or not payload.get(&"season_context", {}).is_empty()):
		return _invalid(&"legacy_extension", "구형 제출은 신규 보스·시즌 수치를 증명하지 못함")
	if schema == SCHEMA_VERSION and (not payload.has(&"boss_kills") or not payload.get(&"season_context", {}) is Dictionary):
		return _invalid(&"missing_field", "보스·시즌 제출값 누락")
	if (
		float(payload.get(&"elapsed_seconds", 0.0)) <= 0.0
		or int(payload.get(&"kills", 0)) < 0
		or int(payload.get(&"boss_kills", 0)) < 0
		or int(payload.get(&"boss_kills", 0)) > int(payload.get(&"kills", 0))
	...
	if String(envelope.get(&"payload_digest", "")) != _payload_digest(normalized, schema):
...
func _normalized_payload(source: Dictionary) -> Dictionary:
	return {
		...
		&"boss_kills": int(source.get(&"boss_kills", 0)),
		&"season_context": _season_context(source.get(&"season_context", {})),
...
func _payload_digest(payload: Dictionary, schema: int = SCHEMA_VERSION) -> String:
	var legacy := "%s|%s|%s|%.3f|%d|%d|%.3f|%d" % [
		payload.get(&"run_id", ""),
	...
	return _digest("%s|%d|%s" % [legacy, int(payload.get(&"boss_kills", 0)), JSON.stringify(_season_context(payload.get(&"season_context", {})))])


func _season_context(value: Variant) -> Dictionary:
	if not value is Dictionary or value.is_empty():
		return {}
	return {
		&"season_id": String(value.get(&"season_id", "")),
		&"starts_at": int(value.get(&"starts_at", 0)),
		&"ends_at": int(value.get(&"ends_at", 0)),
		&"policy_id": String(value.get(&"policy_id", "")),
	}
