...
const CATALOG := preload("res://game/features/map_generation/facility_catalog.gd")
@export var live_csv_url := "https://docs.google.com/spreadsheets/d/1dtQKVZiMf7VRFWrVnaL3BqzR0g4ZgEG6ueH9RIN3xqM/gviz/tq?tqx=out:csv&sheet=Facility&headers=1"
...
func _process(delta: float) -> void:
	...
	if not pending:
		last_error = "시설 시트 요청 실패 · 마지막 정상 데이터 유지"
		catalog_changed.emit(get_snapshot())
...
func _received(result: int, code: int, _headers: PackedStringArray, body: PackedByteArray) -> void:
	...
	if result == HTTPRequest.RESULT_SUCCESS and code == 200 and catalog.load_csv(body.get_string_from_utf8()):
		source = "실시간 Facility · 다음 작전부터 적용"
		last_error = ""
	else:
		last_error = "시설 시트 오류 · 마지막 정상 데이터 유지"
	catalog_changed.emit(get_snapshot())
