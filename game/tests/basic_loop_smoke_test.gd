...
func _verify_key_mapping_and_commercial_vfx() -> bool:
	...
	if int(initial_snapshot.get(&"binding_count", 0)) != 22:
		sandbox.queue_free()
		return _fail("변경 가능한 키 Action 수가 22개가 아닙니다.")
