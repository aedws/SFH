...
func _verify_meta_operation_modules() -> bool:
	...
	if failure_message.is_empty():
		...
		var ranking_identity_test_path := ranking_test_path.replace(".json", "_identity.json")
		var ranking_submission_test_path := ranking_test_path.replace(".json", "_submissions.json")
		for path in [profile_test_path, ranking_test_path, ranking_identity_test_path, ranking_submission_test_path]:
			if FileAccess.file_exists(path):
				DirAccess.remove_absolute(ProjectSettings.globalize_path(path))
		...
		stored_ranking.call(&"submit_run", {
			&"run_id": "smoke-roundtrip", &"success": true,
			&"condition_key": "roundtrip", &"recovered_value": 100,
