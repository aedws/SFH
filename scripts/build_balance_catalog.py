...
LABELS = {
    'energy_capacity_multiplier':'기본 AP 최대/시작량 배율','moving_energy_regeneration_multiplier':'이동 중 AP 자연 회복 배율',
	'required_pieces':'세트 필요 부위 수','amount':'고정 세트 보정 수치',
...
TITLES.update(container_capacity='가방·보호 주머니 확장 단계')
LABELS.update(stage='확장 단계', columns='가로 칸', rows='세로 칸')
...
def build():
    ...
    for path in sorted((ROOT / 'game/features').rglob('*.csv')):
        ...
        for i in valid_rows:
            ...
            if row.get('level'):
                label += ' Lv.' + row['level']
            if path.stem == 'container_capacity':
                label += ' · 가격 미정/구매 잠금' if row.get('policy_status') == 'pending' else ' · 기본 지급' if row.get('stage') == '0' else ' · ' + row.get('policy_status', '')
