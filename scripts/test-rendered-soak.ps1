param(
    ...
    [ValidateRange(0, 3600)][int]$CaptureSeconds = 0
)
...
$dirtyGameTree = [bool](& git -C $repositoryRoot status --porcelain --untracked-files=all -- game)
Write-Output "RENDERED_SOAK_START tier=$Tier seconds=$Seconds base_commit=$commit dirty_game_tree=$dirtyGameTree output=$OutputDirectory"
# No headless, time-scale, movie fixed-time or personal save paths. A visible game window is intentional.
& $executable --path $repositoryRoot --script 'res://game/tests/rendered_soak_test.gd' --log-file $logPath -- "--tier=$Tier" "--seconds=$Seconds" "--output=$OutputDirectory" "--capture-seconds=$CaptureSeconds"
$status = $LASTEXITCODE
...
Write-Output "RENDERED_SOAK_VERIFIED base_commit=$commit dirty_game_tree=$dirtyGameTree tier=$Tier output=$OutputDirectory (coverage, not human acceptance or GPU minimum-spec certification)"
