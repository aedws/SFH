...
func _initialize() -> void:
	...
	catalog["difficulties"]=contracts.get_difficulties()
	...
	files.append("game/features/map_generation/compound_district_layout.gd")
	files.append("game/features/map_generation/compound_space_shape.gd")
	files.append("game/features/operation_contract/data/difficulty.csv")
	...
	for tier in ["small","medium","large"]:
		...
		catalog.tiers[tier]["compound_enabled"]=provider.compound_layout_enabled
		for region: Dictionary in catalog.regions:
			for seed_value in [1,90808,2147483646]:
				var plan: Dictionary=preload("res://game/features/map_generation/regional_district_plan.gd").new().build(catalog.tiers[tier],region.id,seed_value,catalog.facilities)
				if provider.compound_layout_enabled: preload("res://game/features/map_generation/compound_district_layout.gd").new().build(plan,{})
				catalog.fixtures.append({"tier":tier,"plan":plan})
	provider.free()
	for path in files:
