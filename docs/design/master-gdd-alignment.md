...
1. `SmartTargetingPolicy.resolve(densest)`가 중심을 계산한 뒤 공통 target 분기에서 적 좌표로 덮어씁니다. 고정 탐색 반경과 스킬 피격 반경도 다릅니다. N26-03에서 적 3기·동률·사거리 경계로 검증합니다.
...
select_densest_point로 평균 중심을 계산하지만 resolve의 공통 target 분기에서 적 좌표로 덮어씀. 고정 density_radius도 스킬별 피격 반경과 별도. 중심점·반경 전달 회귀 필요.
