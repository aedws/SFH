...
확정도 가중 코드 대응도는 **72.4%**입니다. 하락은 기능 삭제가 아니라 새 요구 범위 확대와 이전 과대 판정 정정입니다.
...
1. **N26-03A 수정 완료:** 중심점을 적 좌표로 덮어쓰던 분기를 수정하고 효과 반경·배율 전달과 실제 실행 회귀를 추가했습니다(구현 커밋 `d50fdb6`). 기존 이웃 군집 평균과 원형 최대 피격 지점은 다르므로 전체 요구는 부분 대응이며 N26-03B로 이어갑니다.
...
2026-09-07 사용자 지시에 따라 판단이 필요 없는 부분부터 N26-03A를 구현했습니다. 아래 정적 대조표의 기준점 이후 변경 근거는 항목별로 추가하며, 전체 최대 피격 요구를 아직 충족하지 않아 72.4%를 임의로 올리지 않습니다. [독립 작업 순서](current-milestone-workline.md#notion-20260907)와 [모듈 계약](../features/smart-targeting.md)을 확인합니다.
...
정적 코드·기존 테스트 소스 대조: **123/170 = 72.4%**. 출시 준비율이나 이번 턴의 실제 플레이 통과율이 아닙니다.

구현 근거 있음 37 / 부분 대응 8 / 규칙 충돌 1 / 신규 미구현 6 / 기획 판단 대기 14
...
노션: **결정 (미구현)** · 코드: **부분 대응** · 작업: **N26-03**

N26-03A(d50fdb6): 중심 덮어쓰기 수정, 효과 반경·배율 전달, 후보 사거리·중복·삭제·동률·실제 실행 회귀 통과. 기존 평균 군집은 원형 범위의 전역 최대 피격을 보장하지 않아 부분 대응 유지; N26-03B에서 보완.

수락 기준: 몹이 가장 뭉쳐 있는 위치의 중심 좌표에 스킬 투하

코드 경계: `game/features/smart_targeting/smart_targeting_policy.gd`

기존 검사 근거: `game/tests/basic_loop_smoke_test.gd`, `game/tests/smart_targeting_center_contract_test.gd`
...
장비 무비용 편집·복원과 스킬 런타임 계측은 있음. 룬 전체 자유 편집/복원의 동등 근거는 부족.
...
최신 기획, GDD v128, 66개 작업, 원본 이관, 코드 대조, 72.4%, 회복 충돌, 혈전, 버서커, 보안 파우치, 심층 진입, 수문장, 키카드, N26, 오너 판단
