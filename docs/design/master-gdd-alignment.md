...
## 9/23 승인 적용 · 현재 우선 기준 {#approved-20260923}

직전 제안에 대한 오너의 ‘진행 시작’으로 세션 시간·Hunter 병존·무손실 가방 이관·파우치 정산·교체 가능한 임시 수치 적용이 승인됐습니다. 실서비스 계정 서버·과금 변경은 제외합니다. 아래9/21~22 충돌/판단 대기 표는 당시 기록이며 동일 항목의 승인 여부는 [현행 실행선](current-milestone-workline.md#approved-20260923)을 우선합니다.

첫 패킷은 **15~20분 권장 생환·20분 오염/Hunter·30분 붕괴**입니다. 전용 자동 계약으로 두 신규 미구현 행을 구현 근거 확보로 옮겨, 추가15고유요구만의 상태는 **구현7·부분5·미구현3**입니다. 기존66행/전체82행 가중 백분율은 이번 패킷에서 새로 계산하지 않습니다. 출처는 아래 공개 원문101블록/82행 스냅샷과9/23 오너 승인, 코드/검사 근거입니다. Notion 기획 체크박스를 변경하지 않았으며 정상 속도 장시간 사람 수락은 별도 잔여입니다.

**9/22 후속 반영:** 공개 원본 해시는 아래9/21과 동일합니다. 장착 룬 정산 전용 회귀·실시간 가방을 반영해 추가15고유요구의 코드 상태는 구현4·부분6·미구현5입니다. 아래2·8·5는9/21 시점 이력이며 전체82행의 백분율은 아직 산정하지 않습니다. [이번 변화·다음 작업](current-milestone-workline.md#decided-20260922).
...
신규16행은 모두 기획상 `결정 (미구현)`입니다. 이는 코드 부재의 증거가 아닙니다. 9/21 [신규16행 코드 대조표](current-milestone-workline.md#delta-audit-20260921)를 작성했고, 수락 조건이 같은 미니 루프2행은 중복1행으로 분리했습니다. 고유15요구 중 코드 구현2·부분8·미구현5이며, 기존 정량 감사 JSON은66행 기준 이력으로 보존합니다. 최신82행 전체 백분율이나 사람 수락 완료로 해석하지 않습니다.

### 무엇이 추가됐고 무엇을 확인해야 하나

| 신규 요구 묶음 | 코드 대조 및 다음 판단 |
|---|---|
| 1~2분 탐색→전투→파밍→결단, 공격 전조0.3~0.5초 | 0.4초 붉은 경고·방향 고정·좁은 판정 구현/회귀 추가. 루프 시간과3초 보상 판단은 별도 계측 필요. [공격 변경](../features/enemies.md#telegraph-20260921) |
| 15~20분 권장 탈출, 20분 이후 오염/추격자, 30분 즉사 | 현행 규모 설정은 소형540초·중/대형600초 목표. 기존 10분 내외 요청과 상충하므로 세션 길이·강제 종료를 이번 표현 수정에 섞지 않음 |
| 탈출 반경 이탈2초 유예 후 초당1초 역행 | **9/21 첫 반영 구현·자동 검증.** 교체 가능한 정책, 두 출구·유예 경계·방어 시작 시간 상한·재진입·HUD·정산1회 회귀를 추가. [동작과 근거](../features/extraction-defense-results.md#decay-20260921). Notion ‘미구현’ 표기는 별도 기획 상태로 보존 |
...
1. 공격/명중/40스킬 표현과 전리품 비교의 병합·배포는 완료했습니다. ‘새 노션 작업 반영 시작’ 지시로 탈출 감쇠를 첫 구현하고 [신규 작업선](current-milestone-workline.md#notion-implementation-20260921)에 기록했습니다.
2. 신규16행 고유 ID/코드 대조 및 공격 전조를 반영했습니다. 다음은 장착 룬 정산 전용 회귀→실시간 가방의 저장/피격/사망 보호→짧은 루프 계측 순서입니다. 전체82행의 가중 진행률은 재산정 전까지 발표하지 않습니다.
3. 개발자 오너 판단 콘솔의 `DEC-N26-0921`에서 세션 길이·가방 마이그레이션·파우치/룬 정산·요원 조건부 수치의 적용 범위를 확인합니다. 기존 구역 밖 무기한 일시정지는 이번 신규2초 유예/역행 요구로 교체했고, 그 밖의 충돌은 유지합니다.
