...
!!! warning "사전 작성 · 아직 구현 완료 아님"
    이 문서는 P4-06 이후의 순서와 완료 조건을 미리 고정합니다. 표의 P5·P6 항목은 코드와 실제 플레이 E2E를 통과하기 전까지 진행률에 포함하지 않습니다.
...
|---|---|---|---|---|
| P5-01 | Character ID·패시브·투입 가격·표현 키 | `CharacterCatalog`, `CharacterInvestmentService`, Presenter | 캐릭터를 바꾸면 패시브와 총비용이 즉시 바뀜 | 선택→견적→출격→실전 패시브 |
...
|---|---|---|---|
| P6-01 | `RankingProvider` 인터페이스 뒤 로컬·온라인 구현 교체 | 온라인/오프라인·동기화 대기 상태가 보임 | 공급자 교체, 네트워크 실패, 로컬 보존 |
| P6-02 | `RunSubmissionEnvelope`, 멱등 키, 검증 큐 | 제출·성공·재시도·거부 이유를 확인 | 중복 제출, 변조, 시간초과, 재시도 |
| P6-03 | `SeasonPolicy`에 기간·참여 조건·종료 스냅샷 | 종료 시각과 참가 규격이 보임 | 시즌 경계·규격 불일치·종료 후 읽기 전용 |
| P6-04 | `SeasonRewardService` 지급과 프로필 장착·표현 분리 | 지급 근거와 칭호·오라 적용을 확인 | 순위 확정→1회 지급→재접속 유지 |
...
- 신원·서명·재시도 정책이 없으면 P6-02 이후를 완료로 표시하지 않습니다.
