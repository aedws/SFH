...
**9/23 후속 · 거점 공급 경계:** 공통 정책→상점 후보/견적/실물 정의→제작 결과 저장 전 재검증을 연결했습니다. G3 희귀 대응은 임시 값이고 기존 고유 스킬·보유 장비는 유지합니다. [계약·검사](../features/p5-hub-progression.md#supply-20260923). 다음은 실제 심층3경로와 전용 드랍이며 이번 공급 상한으로 완료 처리하지 않습니다.

**9/23 최신 · 등급 가방/보호 주머니:** [종류·배치 정책→실물 소유권→복귀 투영→단일 저장](../features/grid-inventory.md#pouch-20260923)을 분리했습니다. 기본36칸·주머니4칸, 보호품 사망 원형 보존/성공 기존 정산, 소켓 왕복 ID 유지. `ContainerCapacity`7행과 Web payload를 연결했고 비용·해금은 원자적 거래입니다. **가격 미승인으로 구매 잠금**, 스캐빈저 추가 공간·정상 장시간 수락은 남습니다. [전용 회귀](../quality/e2e-play-session.md#pouch-20260923) · [다음 심층 공급처](../design/current-milestone-workline.md#next-20260923).

**9/23 실행 기준:** `run_pressure` 정책/시계/적 공급자/정산 조립을 분리했습니다. 20분 강화·Hunter와30분 단일 실패를 전용 계약으로 검증합니다. [코드·수치·경계](../features/extraction-defense-results.md#pressure-20260923) · [실물 메타데이터 보존 이관부터 남은 실행 순서](../design/current-milestone-workline.md#next-20260923). 미정 룬·요원·심층 세부값의 교체 가능한 임시 구현은 승인됐으며 실서비스 계정·과금은 제외합니다. 정상 속도 장시간 사람 수락은 여전히 잔여입니다. 아래 과거 승인 대기 표현과 구현 완료를 구분합니다.

작업·구조·검증 근거를 골라 확인하세요. 최종 판단은 프로젝트 오너가 합니다.

**9/22 후속:** [런 소켓 대상 선택](../features/session-sockets.md#target-selection-20260922)을 선택적 제공자 API로 연결했습니다. 후보 사본→명시적 선택→저장 이후 재검증→선택 인스턴스 소비이며 Game 조립부 변경은 없습니다. 스킬별1~3소켓/4계열 룬은 계속 부분 상태입니다. [전용 회귀·렌더 근거](../quality/e2e-play-session.md#socket-target-20260922).

**9/22 실행 상태:** [전술 적응·자원 수정자 계약](../features/character-selection.md#tactical-adaptation-20260922)과 Sheet/CSV `2026-09-22.1` 반영.  장착 룬 정산 회귀·실시간 가방·[플레이 흐름 계측](../quality/e2e-play-session.md#run-flow-20260922) 반영. **[다음 작업: 정상10분/F와 FUN QA](../design/current-milestone-workline.md#next-20260922)**. 보고서에서 교전·보상 판단·다음 방 이동과 후퇴/미완료를 나눠 확인하세요. 훈련 라이브 동등성 수정은 완료했으며, 양 플랫폼 정상10분·모바일/FUN QA와 기획 충돌 판단은 별도로 남아 있습니다. **아직100% 아님**이며 이전66행 점수를 최신82행 완료율로 사용하지 않습니다.
...
<nav class="sfh-workspace-launcher" aria-label="개발자 작업 바로가기">
  ...
  <a href="../../design/current-milestone-workline/#next-20260922"><small>실행</small><strong>다음 작업</strong><span>확정 패시브와 잔여 수락 조건</span></a>
...
<div data-sfh-distance-lab data-catalog="../../assets/dps-catalog.json">현재 거리 곡선 읽는 중…</div>
...
**2026-09-09 대체 승인: 80.3%** — 구현45·부분1·미구현6·미정14. 첫 GPU 지연을 줄인 청크 렌더러를 TileMapLayer의 대체 구현으로 오너가 수락했습니다. [계산·계약 근거](../design/master-gdd-alignment.md#day-close) · **[현재 실행 순서](../design/current-milestone-workline.md#next-20260910)**. 일반10분 QA는 아직 부분 대응입니다.

| 우선순위 | 대상 | 현재 상태 | 오너가 확인할 완료 조건 |
|---:|---|---|---|
| 다음 실행 | 훈련 동등성 수정·모듈 감사 보완 후 N26-08B → 맵/스킬 체감 | [9/10 현행 패킷](../design/current-milestone-workline.md#next-20260910) | 자동 회귀는 통과. 양 플랫폼 정상10분/F·사람 체감, 동적 조립/초기 선택 책임 검토는 잔여 |
