...
- **이렇게 판단할 수 있습니다 — 미승인 예시:** “임시안: 지정 저항 장비를 입은 요원만 정규 입구 통과. 수문장을 잡으면 키카드 1개를 바닥에 보장 드랍, 가방이 꽉 차면 바닥에 유지. 금고 문을 실제로 연 순간 키카드 1개 소비. 실패는 무차감, 같은 작전에서 열린 문 재통과는 무료.”
...
**그 외 미정 14건**은 [원문별 대조표](../design/master-gdd-alignment.md)와 [요청 목록](#planning-queue)에서 별도로 검토합니다. AP·소켓 등 이미 임시 구현된 규칙은 새 확정안이 없다는 이유로 중단하지 않습니다. HP 키트 전용·청크 렌더러 대체는 승인 완료라 다시 승인 요청하지 않습니다. 기다리는 동안 개발은 배포판 플레이·탈출 F·저장·성능·표시 불일치 검증을 진행합니다.
...
**9/9 도시형 배치:** ‘방으로 연결된 미로’ 대신 도로·건물 정면·서비스 마당으로 길을 읽게 했습니다. 맵 설계실도 같은 골격입니다. 건물 용도·필수 위치는 기존 Facility로 제안하고, 도로 이동 시간·가구 밀도·시야 전환 체감은 별도 수락 기준으로 적어 주세요. [변경·조정점·한계](../features/extraction-district.md#urban-20260909).
