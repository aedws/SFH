...
**이번 변경:** 이제 로비 왼쪽 위 보급 상점에서 산 손상·표준·고성능 상품이 I 가방에 실제 아이템으로 들어옵니다. U/E에 장착하면 품질 배율이 실제 능력치에 적용되고, 해제해도 품질·옵션·소켓 정보가 유지됩니다. 현행 수치는 시험값입니다. 기획자는 손상품 가격/성능 범위와 고성능 옵션·소켓 규칙을 확정하면 됩니다. 서버 검증은 아직 구현 예정입니다.
