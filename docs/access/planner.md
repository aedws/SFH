...
**이번 변경:** 방 네 곳을 깨면 무기·방어구·모듈·파츠를 차례로 발견합니다. F로 주워 I 가방에서 사용하고 살아서 탈출하면 창고에 남깁니다. 시즌이 끝나면 칭호·오라를 받아 출격 마지막 화면의 `시즌 보상`에서 꾸밀 수도 있습니다. 현재는 내 기기에 저장하는 테스트 순위이며 전 세계 경쟁 보상이 아닙니다. Notion의 Phase 체크는 0/6 그대로이고, 코드 구현 완료와 별개입니다.
