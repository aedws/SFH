...
**이번 변경:** 출격 마지막 화면에서 시즌 종료 시간과 참가 조건을 보고, `시즌 기록`으로 이번 주·지난 시즌 결과를 읽을 수 있습니다. 보스를 잡은 횟수도 기록됩니다. 현재는 내 기기에 저장하는 테스트 순위입니다. 주기·동점 규칙은 임시값이고, 계정 경쟁·칭호·오라 지급은 아직 없습니다. Notion의 Phase 체크는 실제로 0/6이며, 위키의 구현 상태와 별개입니다.
