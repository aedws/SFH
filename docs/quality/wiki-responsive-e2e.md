...
- 즉시 페이지 이동 후에도 검색 데이터, 브라우저 플레이, 57개 문서 링크는 `/SFH/` 배포 루트를 유지합니다.
...
추가로 `모듈 규칙` 문서 맵 검색→결과 클릭, 홈 `DEV`→모듈 규칙, 중첩 문서에서 브라우저 플레이를 실제 클릭했습니다. 모두 루트 경로로 이동했고 브라우저 콘솔 오류는 0건이었습니다. 정적 검사는 57개 Markdown·내비게이션·노드 데이터 경로와 네 뷰포트 계약을 strict 빌드 전에 판정합니다.
