...
## 거점 공급 상한 · 2026-09-23 {#supply-20260923}

심층 패킷의 첫 단계인 **새 구매·제작 장비의 공급 제한**입니다. 이미 소유한 장비, 무기 고정 고유 스킬, 방어구 세트, 모듈 슬롯은 바꾸지 않습니다. 심층 구역·수문장·저항 장비 보장 드랍·키카드 금고는 다음 단계이며 아직 완료가 아닙니다.

- 최대 희귀에 해당하는 임시 **G3 상한**입니다. G1~G5와 희귀 명칭의 대응은 교체 가능한 값이며, 등급이 없는 기존 제작 기록/방어구는 G1로 간주합니다.
- 제작 소켓 결과는 **1~2개**입니다. 기존 Recipe의0~1 요청은1개로 적용하고 견적도 실제 범위를 표시합니다.3개 이상 요청·심층 전용·종결 고유 옵션은 축소 판매하지 않고 결제 전에 거부합니다.
- 제작 옵션은 현재 지원하는 화력·순환·생명력·장갑 수치만 허용합니다. 임의의 새 메커니즘이 섞이면 제작하지 않습니다. 무기 정의에 고정된 고유 스킬을 제거한다는 의미가 아닙니다.
- 회전 상점은 정책 위반 품목을 목록/리롤 후보에서 제외합니다. 직접 구매·지급도 검사하며, 공급 후보가 없는 리롤은 돈을 받지 않습니다. 실물 제공자는 실제 연결된 무기/방어구 정의까지 검사합니다.
- 교체한 제작 공급자가 잘못된3소켓 결과를 반환해도 저장·차감 전에 거부합니다. 기존 저장품을 소급 축소하지 않습니다.

`EquipmentSupplyPolicy`는 순수 규칙, `game/core/configs/default_equipment_supply.tres`는 공통 조정점입니다. 제작 Roll→거래 저장, 상점 후보→견적→실물 지급과 구형 상점/제작 경로가 소비합니다. UI는 정책 설명과 유효 범위만 표시합니다. **품질 소켓·제작 기록 소켓·장비 모듈 슬롯·향후 스킬별 룬 소켓은 별개**이며 이번 작업을 스킬별1~3소켓 실행 완료로 세지 않습니다. 거점 수량 지급 모드는 매물 메타데이터를 검증하며 실물 정의 검사는 실물 지급 제공자가 담당합니다.

새 아이템/공급처 목록이 없어 Sheet 탭·CSV 스키마를 늘리지 않았습니다. Recipe는 요청 범위이고 정책 적용 후 범위는 견적에서 확인합니다. 심층 전용 목록을 만드는 후속 단계에는 공유 Sheet→확정 CSV→Web payload→릴리스 검증을 함께 확장합니다.

근거: [공개 GDD](https://wobbly-pawpaw-1ff.notion.site/3d35b728004081eba698e9a00f6ec0ed)의10절 상점/제작 상한과6.1절 심층3경로.9/23 재조회는 v206·101블록이며 저장된9/9 스냅샷과의 차이는 이미 위키에서 대조한 신규 항목입니다. Notion 체크박스는 수정하지 않습니다. [검증](../quality/e2e-play-session.md#supply-20260923).
