...
현행 확정 LootTable의 연구 단지·숙련 이상·중형 이상 방 보상에서 `아크 질주` 스킬이 나올 수 있습니다. 장착 카탈로그의 지역 태그와 실제 드랍 조건은 다릅니다. 비교 패널은 기존→후보 스킬, 대상 슬롯, 현재 키, ENERGY, CD, 충전 수를 한 번에 보여 줍니다.

`R 스킬 교체`를 누르면 기동 가속이 아크 질주로 바뀌고 **현재 지정한 키**를 이어받습니다(기본 3번, K에서 9번으로 옮겼다면 9번). 아크 질주는 입력 방향으로 즉시 이동하며 경로의 적에게 전기 피해를 줍니다. 전기 태그가 없는 활성 무기라면 교체를 거부하고 이유를 표시합니다.

### 2026-09-09 · 교체 후에도 편집 가능한 슬롯

출격 선택과 현장 교체 모두 같은 `SkillBindingService` 계약을 사용합니다. K에는 교체된 스킬이 표시되며 다시 키를 배치할 수 있습니다. 연속 교체 후 거점으로 돌아와도 최근 사용자 키를 유지하고, 다른 스킬과 바꾼 키를 덮어쓰지 않습니다. 임시 스킬의 ID는 파일에 저장하지 않으며 원래 슬롯 소유자의 배치만 저장합니다. 저장 실패는 변경 직전 상태로 복구합니다. [재현·확장·실제 키 검사](../quality/e2e-play-session.md#loot-skill-20260909).
...
| 확정 CSV | `skill_catalog.csv` 4행·`loot_table.csv` 77행(데이터 행 기준) |
