...
현재 확정 데이터는 세 지역 77행입니다. Weapon·Armor·Skill 정의와 Item의 모듈·파츠를 ID로 연결합니다. 2026-09-03에 지역별 방 실물 12종과 보스 4종, 합계 48행을 추가했습니다. 방 네 곳의 무기→방어구→모듈→파츠 순환과 일반 적 확률·보스 2회 추첨은 별도 `FieldLootSpawnPolicy`가 담당합니다.
...
행의 `item_id`는 Item 생명 주기 또는 Weapon·Armor·Skill을 연결한 현장 카탈로그에 존재해야 합니다. 무기·방어구를 Item에 중복 등록하지 않습니다. ID·지역 교차 검증에 실패하면 기존 정상 테이블을 유지하고 새 데이터를 거부합니다.
...
현장 생성·가방 수용·사용·탈출 반출은 [한 판 전리품 획득](field-loot-acquisition.md)에 연결됐습니다. `vault`는 테이블 확장 지점이며 모든 금고가 실물을 드랍한다는 뜻은 아닙니다.
