...
서비스는 가격·해금·선택·태그 적합성과 런 구매 상태만 소유합니다. 장비 내부 상태, 스킬 효과, 프로필 잔액을 직접 수정하지 않습니다. 총비용 차감은 기존 작전 계약이 한 번 수행하고, 조립 실패 시 기존 롤백 계약이 전액 복원합니다.

거점 장비는 런 시작 전에 사본으로 보존합니다. 선택한 펄스 소총과 아크 질주는 전투 세션에만 적용되고, 탈출·사망·조립 실패 뒤에는 거점의 원래 장비와 스킬 배치로 돌아갑니다.
...
소·중·대형×표준·숙련·악몽 9조합, 기본 루프, 캐릭터 비용 1회 합산, 대형 60 FPS CPU 예산도 함께 회귀 검사합니다.
