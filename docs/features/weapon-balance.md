...
거리별 직격 피해와 기획 확정 그래프는 [무기 거리 피해](weapon-distance.md)에서 확인합니다. Weapon 시트 `distance_damage_curve` 열을 같은 실시간→확정 CSV 경로로 읽습니다.

## 현재 구현

현행은 [총기 5종·근접 3종](weapons.md#arsenal-20260909)입니다. 아래 슬롯 표는 기본 지급 장비만 표시합니다. `Weapon!AS`의 `attack_mode`로 탄환/베기/찌르기를 선택하며 새 무기는 행 8~10, 단검/대검 활성화는 행 5~6에 반영했습니다. `A1:AS10`의 1행 변수명·2행 한글 설명·3행 이후 데이터를 유지합니다. 잠금 CSV는 8행, LootTable은 101행입니다.

근접은 `projectiles_per_shot=1`, `burst_count=1`로 고정합니다. `spread_angle_deg`는 베기 전체 각도/찌르기 폭 계산 각도이며 `pierce_count+1`은 최대 타격 인원, `pierce_damage_retention`은 가까운 대상부터 다음 대상에 적용할 배율입니다. 찌르기는 최소 반폭 8px입니다. `projectile_speed_px_sec=1`은 미사용 호환 필드, `projectile_lifetime_sec`는 근접 궤적 표시 시간(0.08~0.3초 제한)입니다. 공격당 한 번만 피해를 적용합니다.
...
로비에서 장비를 세팅한 뒤 작전 게이트 F → 장착 확인 → **3단계 검토·투입**에서 다음 두 버튼을 선택합니다. [기획자 실습 튜토리얼](../getting-started/planner-item-balance-tutorial.md)은 지원 열·새 아이템 연결·CSV 확정을 구분합니다.
