...
현재 런타임 실시간 연결 대상은 `Weapon`, `RunBuff`, `Upgrade`입니다. `Armor`와 `Item`은 정의 데이터 원본으로 유지하며, 성장 수치는 ID로 연결된 `Upgrade` 탭이 담당합니다.
...
- `실시간 테스트`: 공용 `Weapon`, `RunBuff`, `Upgrade` Google Sheet를 기본 3초마다 다시 읽습니다. 네트워크 오류가 나면 마지막 정상값을 유지하거나 확정 CSV로 복구합니다.
