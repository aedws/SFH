...
**9/23 강제 마감:** 30분 붕괴는 기존 사망과 같은 실패 경로를 사용합니다. 공통 실패 조립에서 한 번 정산하고 종료 후 늦은 탈출/사망 신호로 결과를 바꿀 수 없습니다. [시간 정책](extraction-defense-results.md#pressure-20260923).

## 보호 주머니 정산 {#pouch-20260923}

`InventoryRunProtection`이 획득 장부와 거점 복귀 상태를 별도로 만듭니다. 실패 시 **실제로 저장된 주머니 인스턴스만** 손실 장부에서 제외하고 동일 가방 저장 문서의 거점 보관에 옮깁니다. 성공 시에는 기존 `RunSettlementService`의 도면 해금·런 자산 환전을 사용하며 원형 실물을 함께 돌려주지 않습니다. 결과 화면에 보호 개수를 별도로 표시합니다.

거점에서 주머니에 넣어 가져온 실물도 이번 작전의 정산 대상입니다. 가방/소켓 이동을 획득으로 다시 더하지 않고, 같은 아이템의 보호품과 비보호품은 실물 ID로 구분합니다. `run_id` 중복 방지와 미저장 편집 폐기는 유지합니다. 소켓 장착 중인 자산은 주머니 보호가 아닙니다. [사용법·Windows 중단 기록·가격 미확정 경계](grid-inventory.md#pouch-20260923).
