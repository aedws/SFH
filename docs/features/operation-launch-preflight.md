...
장착 장비 보존 정책 `preserve_hub_equipment`를 계획에 기록합니다. 카탈로그 무기 경로·재청구는 제외하고 실제 로비 장비·모듈·파츠 상태만 사용합니다. 스킬 투자와 기존 해금·비용 검증은 유지합니다.
...
- 실제 로비 무기 커스터마이징 유지, 구형 카탈로그 교체 거절, 정상 거점 복원
