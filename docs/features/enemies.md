...
**9/23 시간 압박:** 작전20분부터 기존/신규 일반 적 공격력×1.35·이동×1.15의 임시 오염 배율을 적용하고 별도 Hunter를 한 번 생성합니다. 생성 한도·문/방 클리어와 분리하며 기존50% 추격 보스 스펙은 다시 곱하지 않습니다. [규칙·교체 가능한 수치·검증](extraction-defense-results.md#pressure-20260923).

## 플레이 흐름

### 보고 피하는 공격 · 2026-09-21 {#telegraph-20260921}

잡몹의 즉시 접촉 피해를 **붉은 전조 → 방향 고정 → 지연 타격 → 회복**으로 바꿨습니다. 기본 전조는0.4초이며 시작 프레임에는 피해를 주지 않습니다. 전조 중 적은 추적을 멈추고, 표시된 길이48px·전체 폭20px 안에 플레이어 중심이 남았을 때만 한 번 피해를 줍니다. 옆이나 범위 밖으로 피하면 빗나가며 빗나가도 기존0.75초 공격 간격을 소비합니다. 경고가 플레이어를 계속 따라 돌지 않습니다.

`EnemyAttackPolicy`는 시간·판정 형태, `EnemyAttackTelegraph`는 공격 상태와 같은 도형의 경고 표시, `EnemyCharacter`는 이동·상태 이상·기존 피해/회복 연결을 담당합니다. `configs/default_enemy_attack.tres`로 수치를 교체하며 구성 시 복사해 공유 원본 변경이 진행 중인 공격에 스며들지 않게 합니다. 일반 적은 벽 너머를 공격하지 않고, 기절·9px 이상 밀림·공격 비활성·대상 제거·사망은 전조를 취소합니다. 추격 보스의 기존 문/벽 무시와 보스 상태 이상 정책은 유지합니다. 전조 기능을 끈 명시적 폴백만 이전 접촉 방식을 사용합니다.

새 아이템 목록이나 Sheet 열은 추가하지 않았습니다. 기존 방 스폰·드랍·피해량·난이도 배수와 분리된 정책입니다. 적당 Node2D 하나를 재사용하고 근거리 공격 획득/타격 때만 벽 검사를 수행합니다. 별도 이미지·타이머·매 프레임 노드 생성은 없습니다. [자동 검증과 사람 수락의 경계](../quality/e2e-play-session.md#telegraph-20260921).
