...
거점과 작전에서 `K`를 누르면 키 설정 화면이 열립니다. 이동·대시·기본 공격·상호작용·무기 교체·스킬 1~9·I/U/E/M 메뉴와 K 자체까지 총 22개 Action을 키보드 또는 마우스 버튼으로 바꿀 수 있습니다. 변경값은 즉시 `user://sfh_key_mapping.json`에 저장되어 재실행 뒤에도 유지됩니다.
...
- 자동 검증은 22개 Action, ESC 예약, 중복 교환, JSON 저장, 패널 행 수와 실제 K/ESC/M 입력을 검사합니다.
