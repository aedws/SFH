...
거점과 작전에서 `K`를 누르면 입력 설정 화면이 열립니다. `키 배치` 탭은 이동·전투·메뉴와 현장 장착 R을 포함한 23개 Action의 물리 키를 바꾸고, `스킬 배치` 탭은 장착한 스킬을 허용된 1~9 Action으로 옮깁니다. 두 단계는 독립 저장되므로 물리 키만 바꾸거나 스킬 위치만 바꿀 수 있습니다.
