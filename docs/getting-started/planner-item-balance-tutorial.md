...
[기획자 작업실로 돌아가기](../access/planner.md) · [공용 밸런스 시트 열기](https://docs.google.com/spreadsheets/d/1dtQKVZiMf7VRFWrVnaL3BqzR0g4ZgEG6ueH9RIN3xqM/edit) · [Notion에 결정 남기기](https://wobbly-pawpaw-1ff.notion.site/3d35b728004081eba698e9a00f6ec0ed)

2026-09-04에 실제 시트의 탭과 1·2행을 읽어 코드와 대조했습니다. Weapon의 고유 스킬/고정 옵션 열과 Armor의 고정 옵션 열은 이번 작업에서 추가했으며, 아래 전투 수치 예시는 설명용입니다.

## 9/23 추가 · 가방·보호 주머니 용량

**ContainerCapacity** 탭을 추가했습니다. [가방·주머니 단계 조정 안내](../features/grid-inventory.md#pouch-20260923)를 참고하세요. 1행 변수·2행 한글 설명·3행 이후 7단계이며, 비용 후보는 아직 `pending`/구매 불가입니다. `upgrade_enabled`만 켜고 pending을 남기면 검증이 거부합니다. 가격 승인은 Notion의 값·근거와 오너 확인을 거쳐야 하며 시트 편집 자체가 배포 완료는 아닙니다.

## 9/22 추가 · 요원의 AP 패시브

공용 시트 `Character` Q/R열에서 최대·시작 AP 배율과 이동 중 자연 회복 배율을 조정합니다. 예: 선봉대1.15/1.20, 다른 요원1/1. 1행 이름·2행 설명은 유지하고3행 이후 숫자만 편집하세요(0 초과~5). [전술 적응의 확정 근거·임시 기본값](../features/character-selection.md#tactical-adaptation-20260922)을 먼저 확인합니다. 시트 편집은 라이브 시험용이며 배포는 검증한 CSV/Web payload 잠금이 필요합니다.

계산기에서는 선택 요원을 고른 뒤 ‘지속 이동 가정’을 켜거나 꺼 정지/이동을 비교합니다. 자원 입력값에 패시브를 미리 곱하지 않습니다. 확정 이유는 Notion에 적고 변경 위치를 전달해 주세요.
...
먼저 [맵 설계실](../access/planner.md#map-workbench)에서 시드를 바꿔 기준점이 유지되는지 비교하세요. 공간을 선택하면 피스 종류의 필수 지역·위치·가로/세로·출입 방향을 시험할 수 있습니다. 초안은 자동 공유되지 않으므로 복사해 Notion에 붙이고 오너에게 전달합니다. G~L열은 필수 지역, 방위, 가로/세로 비율, 출입 방향, 보조 추첨 가중치입니다. 금고는 전 지역 필수·추첨 제외를 유지합니다.

1. `facility_id`는 기존 참조가 있으므로 이름을 바꾸지 말고 `display_name`으로 표시명을 바꿉니다.
2. `risk_bonus` 0~2는 중심 시설의 적 수 보정, `cache_weight` 0 초과~10은 현장 자원 배치 가중치입니다. 무조건 얻는 보상량이 아닙니다.
3. `encounter=objective` 행은 정확히 1종, 나머지는 `patrol`. 의료 구역을 추가해도 의료품 전용 드랍이 자동 생성되지는 않습니다. 워프 역할은 개발자 설정 `warp_facility_ids`로 관리합니다.
4. `planner_note`에 `[임시/확정] 사유, 기대 행동, 확인할 시간/비율`을 적고 노션 근거를 연결합니다.
5. 게임의 **실시간 테스트**를 선택하면 30초마다 유효한 행을 읽습니다. 브리핑의 시설 원본 상태를 확인하고 **새 작전**으로 검수합니다. 진행 중인 시설은 바뀌지 않습니다.
6. 오류는 마지막 정상값 유지. 개발자는 `python scripts/sync_facilities.py --verify-live`로 읽기 검사 후, 같은 스크립트를 옵션 없이 실행해 CSV와 Web payload를 함께 확정합니다. `--check`로 일치 검증하고 Web/Windows를 같은 커밋으로 배포합니다.

기존 Weapon·Armor·Item 등은 그대로입니다. 시설마다 새 행동이나 드랍 분류를 만들려면 행만 추가하지 말고 해당 효과의 구현·E2E도 함께 요청하세요.

시트를 바꾸기 전 [DPS 실험실](../tools/dps-lab.md)에서 현재 수치와 시험값을 비교하세요. 적 체력·방어막·목표 처치 시간을 넣으면 그래프가 바뀝니다. 시험값은 시트에 자동 저장되지 않으며, 확정 CSV와 실제 플레이 검증은 기존 절차대로 진행합니다.
