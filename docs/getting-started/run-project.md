...
무기, 내부 성장, 장비 강화, 전리품 생명 주기와 지역·난이도 드랍 테이블의 확정 CSV도 Web PCK에 명시적으로 포함합니다. 브라우저 런타임이 비-Resource CSV 스트림을 제공하지 못하는 환경에서는 원본과 함께 자동 생성한 `EmbeddedCsvPayload`를 읽습니다. 동기화 스크립트와 스모크 테스트가 CSV와 내장 미러의 완전 일치, 원본 경로, 내보내기 포함 계약을 함께 검사하므로 데이터가 빠지거나 어긋나면 작전 조립 전에 빌드를 차단합니다.
