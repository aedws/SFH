...
직전 PR 152의 실행 `33975947990`, main 실행 `33976300564`와 별도 백업 `33976300697`을 새 PR 실행 `34024497953`, main 실행 `34024717246`과 비교했습니다. **작업별 실행 시간을 분 단위로 올림한 추정으로 약 75% 감소**이며, 청구 API의 실사용 합계나 모든 변경의 보장 수치가 아닙니다. 초기 설치·실패 수정·재시도 비용은 이 정상 경로 비교에서 제외했습니다. 자체 러너는 PC 전력·자원을 사용하며, PC가 꺼지면 새 게임 작업의 hosted 대체 실행으로 분 사용량이 늘 수 있습니다.
