...
## 거점 로드아웃 조회 재감사 (2026-08-30)

| 검사 항목 | 결과 | 근거 |
|---|---|---|
| UI 재사용 | 통과 | 거점과 작전이 같은 `GridInventoryWindow`, `EquipmentWorkbench` Scene 사용 |
| 변경 차단 | 통과 | 거점 조립 시 `read_only=true`를 주입하고 모든 장착·성장·강화·개조 경로에서 재검사 |
| 수명 분리 | 통과 | 출격 전에 거점 제공자·패널을 제거하고 새 전투 인스턴스를 조립 |
| 선택 전달 | 통과 | `preferred_weapon_slot` 값만 조립 경계를 넘어 전달하며 장비 내부 Dictionary는 공유하지 않음 |
| 입력 계약 | 통과 | `I`, `U/E`, `Q` 매핑과 열기·전환·출격 유지·귀환 재설치를 자동 검증 |

거점은 장비나 가방 내부 상태를 직접 구현하지 않습니다. `Game`이 기존 모듈의 표시 모드와 수명만 조립하므로 추후 영구 창고 기반 준비 화면으로 교체해도 전투용 UI 계약을 유지할 수 있습니다.
