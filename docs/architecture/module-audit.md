...
| 대형 작전 성능 | 통과 | 1280×720, 방 적 14명, 전기 효과 3개 조건에서 평균 6.888ms·최대 8.163ms·Node 1,310 |

새 규칙을 `Game`의 계산식으로 합치지 않았습니다. `Game`은 계약 생성, 제공자 주입과 Signal 연결만 담당하고 비용·대상·스킬 호환·드랍·페널티·랭킹 계산은 각 기능 폴더에 남습니다. 현재 감사에서 새로 발견된 비모듈 도메인 접근은 없습니다. 사용자 키 설정 저장, 영구 상점 전용 UI와 서버 랭킹은 완성된 공개 계약 위에 제공자를 추가하는 후속 범위입니다.
