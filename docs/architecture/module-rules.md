...
`run_pressure`는 순수 `RunPressurePolicy`와 런별 `RunPressureService`로 분리합니다. Game의 수락된 작전 시계만 전달하고 `_process`/벽시계/별도 저장 타이머를 만들지 않습니다. 서비스는 일반 적 배율·Hunter 생성의 공개 공급자와 종료 요청 신호만 사용하며 재화·가방·프로필을 수정하지 않습니다. Game의 공통 실패 경로가 정산을 소유하고 종료 후 늦은 탈출/사망 신호를 거부합니다.

정책 Resource는 출격 시 복제하고 임시 배율과 확정 시간을 명시합니다. 생성 실패 재시도와 생성 성공 후 처치를 구분합니다. 일반 적 배율은 기존/새 적에 멱등 적용하고 추격자 독립 스펙에는 재적용하지 않습니다. 선택 공급자 제거·기능 토글·일시정지·큰 시계 점프·단일 정산을 `run_pressure_contract_test`로 검사합니다. [계약·수치](../features/extraction-defense-results.md#pressure-20260923).
